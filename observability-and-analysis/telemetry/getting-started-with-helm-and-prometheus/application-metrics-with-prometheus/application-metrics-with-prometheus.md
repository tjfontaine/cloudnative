...
Well, now that you have your monitoring infrastructure in place, it's time to utilise it to actually get some visibility in to your applications!
...
As above, You'll need an OKE cluster configured (watch those network rules!) with Prometheus. If you don't already have this, I recommend following the comprehensive [OKE quick start documentation](https://www.oracle.com/webfolder/technetwork/tutorials/obe/oci/oke-full/index.html) to launch an OKE cluster, then the [first part of this tutorial series](https://gitlab-odx.oracledx.com/cloudnative/devcenter/tree/master/content/prometheus.html) to get your Prometheus infrastructure in place.

## Application Metrics?

Before we get in to the process for deploying an application integrated with Prometheus, lets take a look at what application metrics actually are, and why we'd be interested in them.

Lets assume we have a simple HTTP API server application deployed on our cluster. Out of the box with Kubernetes, we get some visibility on how much resource its pod is consuming, and we can quickly access the stdout/stderr logs from the container with `kubectl logs -f <pod name>`. 

If we wanted to access these logs easier, We could set up an EFK or ELK logging stack to pull those logs in to a central place. We could also configure this logging stack to aggregate some metrics around those log files, e.g "Requests per minute"
...
But what if we want to gather metrics _inside_ of our application? How many times have we raised a certain exception? What was the response time of a certain external microservice call? How long is it taking to resolve DNS internally?

Sure, we could answer some of these questions by adding some kind of [Service Mesh](https://istio.io/docs/concepts/what-is-istio/) between our microservices, but to achieve this without adding extra complexity to your infrastructure and to keep the freedom to decide exactly what information inside of your application is useful, we can modify our application to include a metrics endpoint that Prometheus can call.

## Don't call us, We'll Call you.

Note that I said Prometheus will call us. OUr application will expose a /metrics http endpoint that Prometheus will hit to gather data. This means that we don't need to include information on a Prometheus server to send data to inside of our application, nor worry about disabling it in development environments that aren't using Prometheus. 

The endpoint is there to be consumed by whatever wants to consume it. 

## Lets get _GO_ing with our Demo application

[ಠ_ಠ]

### Go Go Go

For the purpose of this tutorial, we're going to take a very simple Go application that serves a message over HTTP, and learn how to expose a metrics endpoint to track the number of times our user-facing endpoint was called, which we'll do by implementing a few Go libraries.

I may have already mentioned, but we're using Go for this tutorial. However, you don't have to! There are [Prometheus libraries for many languages](https://prometheus.io/docs/instrumenting/clientlibs/) that can be used to achieve application metrics.
...
For the sake of simplicity, I've packed this appliction in to a Docker Image for you. You can pull it from `iad.ocir.io/oracle-cloudnative/prometheus-app-monitor:no-monitoring`.
...
You can launch this Application Image on your Kubernetes cluster with the following Service & Deployment configuraiton:
...
Ok, At this point you should have the demo application running on your cluster. However, the application doesn't expose any metrics.
...
Before we package up this application in Docker image, lets take a look at the /metrics endpoint. You can do this by running the Go application on your local machine with:
...
Again, I've packaged up this image and pushed to to a public OCIR repo for ease of use. This one can be found at `iad.ocir.io/oracle-cloudnative/prometheus-app-monitor:metrics`.
...
_Again, you can use a Dockerfile if you want to build this verison of the application yourself. However, this time there are some Go dependencies you need to pull in. Running `docker build .` from this repository will make it easy for you: https://github.com/riceo/oracle-demo-prometheus-metrics_

## Observing your observables

Ok, at this point you should now have your application running on your Kubernetes cluster, with a Prometheus metrics endpoint exposed. Now it's time to configure Prometheus to poll and display those metrics!
...
In order to do this we will need to modify our Kubernetes Service for the application, and add a new [Prometheus Operator](https://coreos.com/blog/the-prometheus-operator.html) definition to tell Prometheus that the demo application exposes a metrics endpoint, and that we want to pull metrics from it.
...
We've done everything we need to to get our application exposing a /metrics endpoint, with Prometheus configured to poll for metrics from it. How do we test this?

Hit the Prometheus Web interface on your cluster ([reference the first part of this post series](https://gitlab-odx.oracledx.com/cloudnative/devcenter/tree/master/content/prometheus.html) if you are unsure how), and add "rate(requests_total[1m])" in to the query box, then select "Graph". 
...
This will make 10,000 requests to your application with a concurrency of 10 workers, which should make our graph look a little bit more like:
