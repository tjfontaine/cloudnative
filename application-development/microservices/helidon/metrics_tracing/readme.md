...
In this tutorial you will deploy a microservice application created using the the Helidon framework to a local Docker environment and Kubernetes cluster, and collecting metrics and tracing data from that application. 
...
[Here](https://helidon.io/docs/latest/#/getting-started/01_prerequisites) is an updated list of prerequisites for using Helidon.

## Download the Application 

1. Begin by cloning our repository to get access to the quickstart-se application:

```
git clone https://github.com/oracle/cloudnative.git
```
...
What use are metrics without a tool to consume them? In this lab we will use a tool called [Prometheus](
...
3. Once Prometheus is running, access the console (e.g. `localhost:9090/graph`) and click `Graph`. Enter `application:accessctr` then click `Execute`.
...
The Helidon application has been instrumented to share tracing data. By default the application is configured to connect to Zipkin at `http://localhost:9411`. This is configured in `application.yaml`. In this example you are deploying to a Docker container. In `application.yaml` change the zipkin.endpoint to `zipkin.endpoint: "http://docker.for.mac.localhost:9411"`. After doing so you will need to repackage the application (`mvn package`), rebuild the Docker Image (`docker build -t quickstart-se target`), and restart the Docker container (`docker run --rm -p 8080:8080 quickstart-se:latest`). 
...
2. In the quickstart-se application, navigate to the app.yaml in the src/main/k8s directory. Port 31000 has been whitelisted to allow for Google OAuth to work with the application. Update the nodePort in app.yaml to make sure the application is deployed on the proper NodePort. The ports section of your app.yaml should look like this: 
...
```
kubectl create -f https://raw.githubusercontent.com/oracle/cloudnative/tree/master/application-development/microservices/helidonmicroservices/helidon/metrics_tracing/zipkin.yaml
```

This will install Zipkin on your cluster with the NodePort of 31001 and the port of 9411. 

2. As mentioned before, the application has been instrumented to share tracing data. By default the application is configured to connect to Zipkin at `http://localhost:9411`. This is configured in `application.yaml`. In this example you are deploying to a Kubernetes cluster. In `application.yaml` change the zipkin.endpoint to `zipkin.endpoint: "http://zipkin:9411"`. 

Remember, you will have to repackage the application with `mvn package` and rebuild the Docker image with `docker build -t quickstart-se target` every time the application is modified. 
